...
export const verifyRazorpayPayment = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: "Order processed, stock updated, and email confirmation sent." });
...
// ==========================================
// NEW CASH ON DELIVERY (COD) CONTROLLER
// ==========================================
export const createCodOrder = async (req, res) => {
  ...
  try {
    ...
    // 3. Stock verification & decrement
    for (const item of cartItems) {
      const productRes = await client.query("SELECT count FROM products WHERE id = $1 FOR UPDATE;", [item.product_id]);
    ...
    // 4. Fetch delivery address details
    const addressQuery = "SELECT * FROM addresses WHERE id = $1 LIMIT 1;";
    const { rows: addressRows } = await client.query(addressQuery, [address_id]);
    ...
    // 7. Insert individual order items
    const insertItemQuery = `
      INSERT INTO order_items (order_id, product_id, name, price, quantity, image, selected_size)
      VALUES ($1, $2, $3, $4, $5, $6, $7);
    `;
    ...
    return res.status(200).json({ message: "COD Order placed successfully!", orderId: codOrderId });
