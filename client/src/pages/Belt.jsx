...
const Belts = () => {
  ...
  const [maxPrice, setMaxPrice] = useState(10000) // Default standard ceiling boundary for premium gear hardware
  const [sortBy, setSortBy] = useState('featured')

  // Synchronize category records live from the API setup
  useEffect(() => {
    const fetchBeltsInventory = async () => {
      setIsLoading(true)
      try {
        ...
        if (response.ok) {
          ...
          // CRITICAL LAYER: Filter strictly to show only "Belts" from your database records
          const beltsOnly = data.filter(product => product.category.toLowerCase() === 'belt')
          setProducts(beltsOnly)

          // Set the slider's default dynamic ceiling boundary based on the highest-priced belt
          if (beltsOnly.length > 0) {
            const peakPrice = Math.max(...beltsOnly.map(p => Number(p.offerPrice || p.originalPrice || 0)))
            setMaxPrice(peakPrice > 0 ? peakPrice : 5000)
          }
        }
      } catch (err) {
        console.error("Failed synchronization pipeline connection to API endpoints:", err)
        toast.error("Failed to load tactical hardware index ledger.")
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const handleAddToCart = async (product, token, navigate) => {
    ...
    try {
      ...
      toast.error("Critical network dropout. Hardware loadout sync failed.", { id: syncToastId });
    }
  };
        ...
        <div className="max-w-7xl mx-auto relative z-10 mt-6">
          ...
          <div className="space-y-4 mb-12 text-left border-b border-white/5 pb-8">
              <div className="inline-flex items-center gap-2 text-[10px] font-black tracking-[0.3em] uppercase bg-white/5 border border-white/10 px-4 py-1.5 rounded-full text-lime-accent">
                <Sparkles className="w-3 h-3 text-lime-accent" /> Technical Equipment
              </div>
              <h1 className="text-4xl md:text-6xl font-black uppercase tracking-wider">
                Belts <span className="text-lime-accent font-light">Hardware</span>
              </h1>
          </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center bg-black/30 border border-white/10 rounded-xl px-3 py-2.5 focus-within:border-lime-accent transition-colors">
                  ...
                  <input 
                    ...
                    placeholder="Search utility hardware..." 
              ...
              {isLoading ? (
                ...
                <div className="border border-dashed border-white/10 bg-white/[0.01] rounded-2xl p-20 text-center backdrop-blur-md">
                  ...
                  <p className="text-xs text-white/40 max-w-xs mx-auto mt-1 leading-relaxed">
                    No items match within this tactical equipment layer parameters profile layout.
                  </p>
                </div>
              ) : (
                    ...
                    return (
                      ...
                      >
                        <div className="w-full aspect-[4/5] rounded-xl overflow-hidden bg-black/40 relative">
                          <img 
                            src={product.images && product.images[0] ? product.images[0] : "https://images.unsplash.com/photo-1624222247344-550fb8ef986d?w=500"} 
                            alt={product.name} 
                        ...
                        <div className="pt-4 flex flex-col justify-between flex-grow space-y-4">
                          <div className="space-y-1">
                            ...
                            <p className="text-[11px] text-white/40 line-clamp-2 border-t border-white/5 pt-1.5 mt-1 leading-relaxed">
                              {product.description || 'Premium modular utility structural fastening apparatus.'}
                            </p>
                          </div>
