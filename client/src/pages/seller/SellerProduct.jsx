...
const SellerProduct = () => {
  ...
  const [colorSpecificFiles, setColorSpecificFiles] = useState({})
  const [colorSpecificPreviews, setColorSpecificPreviews] = useState({})
  ...
  const fetchProducts = async () => {
    ...
    try {
      const fetchUrl = userRole === 'admin' 
        ? API_BASE_URL 
        : `${API_BASE_URL}?sellerId=${currentUserId}`
      ...
      if (response.ok) {
        ...
        toast.error("Failed to sync inventory from database.")
      }
    } catch (err) {
  ...
  const handleColorImageUpload = (colorKey, e) => {
    ...
    if (currentFiles.length + files.length > 5) {
      toast.error("You can only upload up to 5 images per color variant.")
      return
    }
    ...
    toast.success(`Staged ${files.length} variant views for: ${colorKey.replace("Color: ", "")}`)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    if (trackingUploadFiles.length > 15) {
      ...
      toast.error("Total image attachments cross maximum limit (Max 15).")
      return
    }
  ...
  const handleDelete = async (id) => {
    toast((t) => (
      <div className="flex flex-col gap-3 text-xs p-1 text-left">
        ...
        <p className="text-white/60">Are you sure you want to permanently erase this record?</p>
      ...
      duration: 6000,
      position: 'top-center',
      style: { background: '#0A224E', border: '1px solid rgba(255,255,255,0.1)' }
    })
  }
  ...
  return (
    <div className="space-y-8 text-white">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-white/10 pb-6">
        <div>
          <h2 className="text-xl sm:text-2xl font-black uppercase tracking-wider">
            <span className='text-lime-400'>{userRole === 'admin' ? 'Admin' : 'Seller'}</span> Product Catalog
          </h2>
          <p className="text-xs text-white/50 font-medium mt-1">Manage, modify, and publish products for your store listing.</p>
        </div>
        <button
          onClick={() => { resetForm(); setIsModalOpen(true); }}
          className="flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-xs font-bold uppercase tracking-wider bg-lime-400 text-slate-950 hover:shadow-[0_4px_25px_rgba(165,206,0,0.35)] transition-all cursor-pointer self-start sm:self-auto font-black"
        >
      ...
      {isLoading ? (
        <div className="text-center py-24 text-xs font-mono tracking-widest text-lime-400 uppercase animate-pulse">
          Synchronizing Live Inventory...
        </div>
      ) : products.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center p-12 border border-dashed border-white/10 rounded-3xl bg-white/[0.02]">
          <Layers className="w-12 h-12 text-white/20 mb-4" />
          <p className="text-sm font-bold uppercase tracking-wider text-white/40">No Products Found For This Account</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.map((product) => (
            <div key={product.id} className="bg-white/[0.03] border border-white/10 rounded-2xl overflow-hidden flex flex-col justify-between backdrop-blur-md shadow-xl hover:border-white/20 transition-all duration-300">
              
              <div className="h-48 w-full bg-black/40 relative overflow-hidden group">
                <img 
                ...
                />
                <span className="absolute top-3 left-3 text-[9px] font-black uppercase tracking-widest bg-black/80 text-lime-400 border border-white/10 px-2.5 py-1 rounded-full">
                  {product.category}
                </span>

                {product.isFeatured && (
                  <span className="absolute bottom-3 left-3 text-[9px] font-black uppercase tracking-widest bg-lime-400 text-slate-950 border border-lime-400 px-2 py-0.5 rounded flex items-center gap-1 shadow-md font-bold">
                ...
                <span className="absolute top-3 right-3 text-[9px] font-black uppercase tracking-widest bg-black/80 text-white border border-white/10 px-2.5 py-1 rounded-full flex items-center gap-1">
                  <Package className="w-3 h-3 text-lime-400" /> Stock: {product.count}
                </span>
              </div>

              <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                <div className="space-y-1">
                  <h4 className="font-bold text-sm text-white tracking-wide line-clamp-1">{product.name}</h4>
                  <p className="text-xs text-white/50 line-clamp-2 font-medium">{product.description}</p>
                </div>

                {product.sizes && product.sizes.length > 0 && (
                  <div className="flex flex-wrap gap-1 items-center">
                    <span className="text-[9px] font-bold tracking-wider uppercase text-white/40 mr-1">Specs:</span>
                ...
                <div className="flex items-baseline gap-2 border-t border-white/5 pt-3">
                  ...
                  <span className="text-[10px] text-white/50 ml-auto self-center bg-white/5 px-2 py-0.5 border border-white/10 rounded">
                    Branch: ₹{product.branch_admin_price !== undefined ? product.branch_admin_price : product.branchAdminPrice || 0}
                  </span>
                </div>
              </div>

              <div className="px-5 pb-5 pt-2 grid grid-cols-3 gap-2 border-t border-white/5 bg-black/20">
                <button onClick={() => setViewProduct(product)} className="flex items-center justify-center gap-1.5 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider border border-white/10 bg-white/5 text-white/80 hover:bg-white/10 hover:text-white transition-all cursor-pointer">
                  <Eye className="w-3.5 h-3.5" /> <span>View</span>
                </button>
                <button onClick={() => handleEditTrigger(product)} className="flex items-center justify-center gap-1.5 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider border border-blue-500/20 bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 transition-all cursor-pointer">
                  <Edit2 className="w-3.5 h-3.5" /> <span>Edit</span>
                </button>
                <button onClick={() => handleDelete(product.id)} className="flex items-center justify-center gap-1.5 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider border border-red-500/20 bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-all cursor-pointer">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md">
          <div className="bg-[#0A224E] border border-white/10 w-full max-w-2xl rounded-3xl shadow-2xl relative flex flex-col max-h-[85vh] text-left">
            ...
            <form id="sellerProductForm" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-wider text-white/60">Product Name *</label>
                  <input 
                    type="text" required placeholder="e.g., Elite Sports Shoes" value={name}
                    onChange={(e) => setName(e.target.value)}
                    className="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-xs font-medium text-white focus:outline-none focus:border-lime-400 text-left"
                  />
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-wider text-white/60">Select Category *</label>
                  <select
                    required
                    value={category}
                    onChange={(e) => { 
                    ...
                    className="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-xs font-bold text-white uppercase tracking-wider focus:outline-none focus:border-lime-400 transition-colors cursor-pointer"
                  >
                    <option value="" disabled className="bg-[#0A224E] text-white/40">-- SELECT CATEGORY --</option>
                    {availableCategories.map((cat) => (
                      <option key={cat.id} value={cat.name} className="bg-[#0A224E] text-white">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-wider text-white/60">Product Description</label>
                <textarea 
                  rows="2" placeholder="Provide product technical description specifications..." value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  className="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-xs font-medium text-white focus:outline-none focus:border-lime-400 resize-none text-left"
                />
              </div>
                        ...
                        <>
                          {colorTabValues.length > 0 && (
                            <div className="space-y-4 border-b border-white/5 pb-3">
                              <label className="text-[10px] font-black uppercase text-white/50">Step 1: Select Available Color Variants</label>
                                  ...
                                  return (
                                    <button
                                      ...
                                      className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border text-xs font-bold transition-all ${
                                        isChecked ? 'bg-lime-400 text-slate-950 border-lime-400 shadow-md' : 'bg-white/5 border-white/10 text-white/60'
                                      }`}
                                    >
                                      ...
                                      return (
                                        <div key={idx} className="bg-black/30 p-4 rounded-xl border border-white/5 space-y-3">
                                          ...
                                          <div className="flex flex-wrap gap-2 items-center">
                                            ...
                                            {previews.length < 5 && (
                                              <div className="relative w-14 h-14 rounded-lg bg-white/5 border border-dashed border-white/20 hover:border-lime-400/50 transition-colors flex flex-col items-center justify-center shrink-0 cursor-pointer">
                                                <input 
                                                ...
                                                <span className="text-[8px] mt-1 text-white/40 font-bold">ADD</span>
                                              </div>
                                            )}
                                          </div>
                                        </div>
                          ...
                          {normalAttributes.map((attrName, index) => {
                                ...
                                <div key={index} className="flex items-center gap-2 py-1">
                                  <button
                                    ...
                                    className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-[11px] font-bold ${
                                      isChecked ? 'bg-lime-400/10 border-lime-400 text-lime-400' : 'bg-black/30 border-white/5 text-white/50'
                                    }`}
                                  >
                                    {isChecked ? <CheckSquare className="w-3.5 h-3.5 text-lime-400" /> : <Square className="w-3.5 h-3.5 text-white/30" />}
                                    <span>{attrName}</span>
                                  </button>
                                </div>
                              )
                            ...
                            return (
                              <div key={index} className="space-y-1.5">
                                <label className="text-[10px] font-black uppercase tracking-wider text-white/50 text-left block">{attrName}</label>
                                <input 
                                  type="text"
                                  ...
                                  className="w-full bg-black/20 border border-white/10 rounded-xl px-3 py-2.5 text-xs text-white focus:outline-none focus:border-lime-400 text-left"
                                />
                              </div>
                            )
                          })}
              ...
              {isClothingCategory && (
                <div className="space-y-2 p-4 bg-lime-400/5 rounded-2xl border border-lime-400/10">
                      ...
                      return (
                        <button
                          ...
                          className={`px-4 py-2 rounded-xl text-xs font-mono font-bold tracking-wider border transition-all cursor-pointer ${
                            isChecked ? 'bg-lime-400 text-slate-950 border-lime-400 font-black shadow-md' : 'bg-white/5 text-white/60 border-white/10'
                          }`}
                        >
                          {sz}
                        </button>
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-wider text-white/60 text-left block">Additional General Images (Optional)</label>
                <div className="flex flex-col items-center justify-center p-6 border border-dashed border-white/10 rounded-xl bg-black/20 hover:border-white/20 relative cursor-pointer group">
                  ...
                  <Image className="w-8 h-8 text-white/40 group-hover:text-lime-400 transition-colors mb-2" />
                  <span className="text-xs font-bold text-white/70">Click to upload general views</span>
                </div>
                {baseImages.length > 0 && (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-wider text-white/60 text-left block">Original Price *</label>
                  <div className="relative flex items-center">
                    <span className="absolute left-4 text-xs font-mono text-white/40">₹</span>
                    <input 
                      type="number" required placeholder="2499" value={originalPrice}
                      onChange={(e) => setOriginalPrice(e.target.value)}
                      className="w-full bg-black/20 border border-white/10 rounded-xl pl-8 pr-4 py-3 text-xs font-mono text-white focus:outline-none focus:border-lime-400 text-left"
                    />
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-wider text-white/60 text-left block">Offer Price *</label>
                  <div className="relative flex items-center">
                    <span className="absolute left-4 text-xs font-mono text-white/40">₹</span>
                    <input 
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-wider text-white/60 text-left block">Branch Admin *</label>
                  <div className="relative flex items-center">
                    <span className="absolute left-4 text-xs font-mono text-white/40">₹</span>
                    <input 
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-wider text-white/60 text-left block">Stock Count *</label>
                  <input 
                    type="number" required min="0" placeholder="50" value={count}
                    onChange={(e) => setCount(e.target.value)}
                    className="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-xs font-mono text-white focus:outline-none focus:border-lime-400 text-left"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 bg-black/20 border border-white/5 p-4 rounded-xl">
                <input 
                  type="checkbox" id="sellerFeaturedCheckbox" checked={isFeatured}
                  onChange={(e) => setIsFeatured(e.target.checked)}
                  className="w-4 h-4 rounded accent-lime-400 bg-black/40 border-white/10 cursor-pointer"
                />
                <label htmlFor="sellerFeaturedCheckbox" className="text-xs font-bold uppercase tracking-wider text-white/80 cursor-pointer select-none">
                  Show this product in Featured Products list
                </label>
              </div>
            </form>

            <div className="border-t border-white/10 p-6 flex items-center justify-end gap-3 shrink-0 bg-[#0A224E] rounded-b-3xl">
              <button type="button" onClick={() => setIsModalOpen(false)} className="px-5 py-3 rounded-xl text-xs font-bold uppercase tracking-wider border border-white/10 bg-white/5 text-white/80 hover:bg-white/10 cursor-pointer">Cancel</button>
              <button form="sellerProductForm" type="submit" className="px-5 py-3 rounded-xl text-xs font-bold uppercase tracking-wider bg-lime-400 text-slate-950 hover:shadow-[0_4px_20px_rgba(165,206,0,0.25)] font-black cursor-pointer">
      ...
      {viewProduct && (
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-black/85 backdrop-blur-md">
          <div className="bg-[#0A224E] border border-white/10 w-full max-w-lg rounded-3xl p-6 shadow-2xl relative space-y-6 text-left">
            
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
              <div>
                <span className="text-[9px] font-black uppercase tracking-widest text-lime-400 font-mono">Product Inspection Mode</span>
                <h3 className="text-base font-black uppercase tracking-wider text-white">{viewProduct.name}</h3>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto rounded-xl">
              {viewProduct.images && viewProduct.images.map((img, idx) => (
                <div key={idx} className="relative">
                  <img src={img} alt={`Asset View ${idx}`} className="w-full h-32 object-cover rounded-xl border border-white/5 bg-black/20" />
            ...
            <div className="space-y-4 text-xs font-medium">
              <div className="flex justify-between items-center bg-black/20 p-3 rounded-xl border border-white/5">
                <span className="text-white/40 uppercase font-bold text-[10px]">Showcase Level:</span>
                <span className={`font-black uppercase tracking-wider ${viewProduct.isFeatured ? 'text-lime-400' : 'text-white/40'}`}>
                  {viewProduct.isFeatured ? '★ Featured Product' : 'Standard Product'}
                </span>
              </div>

              <div className="flex justify-between items-center bg-black/20 p-3 rounded-xl border border-white/5">
                <span className="text-white/40 uppercase font-bold text-[10px]">Category Layer:</span>
                <span className="text-lime-400 font-black uppercase tracking-wider">{viewProduct.category}</span>
              </div>

              <div className="flex justify-between items-center bg-black/20 p-3 rounded-xl border border-white/5">
                <span className="text-white/40 uppercase font-bold text-[10px]">Stock Available:</span>
                <span className="text-white font-mono font-bold">{viewProduct.count} units</span>
              </div>

              <div className="flex justify-between items-center bg-black/20 p-3 rounded-xl border border-white/5">
                <span className="text-white/40 uppercase font-bold text-[10px]">Branch Admin Price:</span>
              ...
              <div className="space-y-1.5">
                <span className="text-white/40 uppercase font-bold text-[10px]">Product Specifications / Sizes Bound:</span>
                <div className="flex flex-wrap gap-1 bg-black/10 border border-white/5 p-3 rounded-xl">
                  {viewProduct.sizes && viewProduct.sizes.length > 0 ? (
                    viewProduct.sizes.map((sz, idx) => (
                      <span key={idx} className="text-[10px] bg-lime-400/10 border border-lime-400/20 text-lime-400 px-2 py-0.5 rounded font-mono font-bold">
                        {sz.split('__imgIdx:')[0]}
                      </span>
                    ))
                  ) : (
                    <span className="text-white/30 italic">No custom parameters matched.</span>
                  )}
                </div>
              </div>

              <div className="space-y-1.5">
                <span className="text-white/40 uppercase font-bold text-[10px]">Product Description:</span>
                <p className="bg-black/10 border border-white/5 p-3 rounded-xl text-white/80 tracking-wide leading-relaxed font-normal">{viewProduct.description}</p>
              </div>

              <div className="flex justify-between items-center border-t border-white/10 pt-4">
                <span className="text-white/40 uppercase font-bold text-[10px]">Price Details:</span>
