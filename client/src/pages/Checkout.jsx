...
const Checkout = () => {
  ...
  // Master Submission Handler for both Online and COD
  const handlePlaceOrder = async () => {
    ...
    if (paymentMethod === 'cod') {
      const codToastId = toast.loading("Placing Cash on Delivery order...")
      try {
        const res = await fetch(`${import.meta.env.VITE_APP_BASE_URL}/api/auth/payment/cod`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${token}`
        ...
        if (res.ok) {
          toast.success("Order placed successfully via COD!", { id: codToastId, duration: 3000 });
          setIsProcessingRedirect(true);
          setTimeout(() => {
            navigate("/orders");
          }, 1200);
        } else {
          toast.error(data.message || "Failed to place COD order.", { id: codToastId });
        }
      } catch (err) {
        console.error("COD error:", err);
        toast.error("Network error processing COD order.", { id: codToastId });
      }
    } else {
      ...
      <div className="bg-gray-50 text-gray-900 min-h-screen py-8 px-4 md:px-8 font-sans">
        ...
        {isProcessingRedirect && (
          <div className="fixed inset-0 z-50 bg-white/80 backdrop-blur-md flex items-center justify-center">
            <EcommerceLoader message="Order Confirmed! Redirecting..." />
          </div>
        )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-gray-500">Cards, UPI, Netbanking (Razorpay)</p>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-gray-500">Pay cash upon delivery handoff</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="lg:col-span-5 bg-white border border-gray-100 rounded-2xl p-5 shadow-sm space-y-5 sticky top-24">
              ...
              <div className="pt-2 border-t border-gray-100 space-y-1.5 text-center">
                <div className="flex items-center justify-center gap-1.5 text-[10px] text-gray-500 font-bold uppercase tracking-wider">
                  ...
                  <span>{paymentMethod === 'cod' ? 'Cash on Delivery Guaranteed' : 'Secured by Razorpay Gateway'}</span>
                </div>
