...
const MarketLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Set auth storage keys
      localStorage.setItem("token", res.data.token);
      localStorage.setItem("userRole", "marketer");
      localStorage.setItem("userName", marketerData.name);
      localStorage.setItem("user", JSON.stringify(marketerData));
      localStorage.setItem("marketer", JSON.stringify(marketerData));

      toast.success(`Welcome back, ${marketerData.name}!`);
        ...
        <div className="flex flex-col items-center text-center space-y-3 mb-8 translate-z-3d">
          ...
          <div>
            <h2 className="text-xl font-black uppercase tracking-widest text-white">
              AVG <span className="text-lime-400 font-light">MARKETING</span>
            </h2>
            <p className="text-white/30 text-[9px] font-black tracking-[0.25em] uppercase mt-1">
              Marketer Partner Portal
            </p>
          </div>
        </div>
        ...
        <div className="space-y-5 translate-z-3d">
          ...
          <div className="pt-4 text-center">
            <p className="text-xs text-white/30 font-medium">
              ...
              <Link to="/marketing/signup" className="text-lime-400 font-bold hover:underline transition-all">
                Register Now
              </Link>
            </p>
          </div>
