...
import { toast } from 'react-hot-toast' // <-- Import the toast asset engine
...
const Watches = () => {
  ...
  const [maxPrice, setMaxPrice] = useState(150000) // Elevated default ceiling for premium timepieces
  const [sortBy, setSortBy] = useState('featured')

  // Synchronize category records live from the API setup
  useEffect(() => {
    const fetchWatchesInventory = async () => {
      setIsLoading(true)
      try {
        ...
        if (response.ok) {
          ...
          // CRITICAL LAYER: Filter strictly to show only "Watches" from your database records
          const watchesOnly = data.filter(product => product.category.toLowerCase() === 'watch')
          setProducts(watchesOnly)

          // Set the slider's default dynamic ceiling boundary based on the highest-priced watch
          if (watchesOnly.length > 0) {
            const peakPrice = Math.max(...watchesOnly.map(p => Number(p.offerPrice || p.originalPrice || 0)))
            setMaxPrice(peakPrice > 0 ? peakPrice : 75000)
          }
        }
      } catch (err) {
        console.error("Failed synchronization pipeline connection to API endpoints:", err)
        toast.error("Failed to load horological matrix ledger node.")
      } finally {
        setIsLoading(false)
      }
    }

    fetchWatchesInventory()
  }, [])
  ...
  const handleAddToCart = async (product, token, navigate) => {
    ...
    // Trigger loading sequence tracker toast instance
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`Sync failed: ${errData.message}`, { id: syncToastId });
      }
    } catch (err) {
      console.error("Cart synchronization error pipeline:", err);
      toast.error("Critical network dropout. Vault loadout sync failed.", { id: syncToastId });
    }
  };
        ...
        <div className="max-w-7xl mx-auto relative z-10 mt-6">
          ...
          <div className="space-y-4 mb-12 text-left border-b border-white/5 pb-8">
              <div className="inline-flex items-center gap-2 text-[10px] font-black tracking-[0.3em] uppercase bg-white/5 border border-white/10 px-4 py-1.5 rounded-full text-lime-accent">
                <Sparkles className="w-3 h-3 text-lime-accent" /> Horology Timepieces
              </div>
              <h1 className="text-4xl md:text-6xl font-black uppercase tracking-wider">
                Watches <span className="text-lime-accent font-light">Vault</span>
              </h1>
          </div>
            ...
            <div className="lg:col-span-3 lg:sticky lg:top-28 lg:max-h-[calc(100vh-160px)] lg:overflow-y-auto space-y-8 bg-white/[0.02] border border-white/5 rounded-2xl p-6 backdrop-blur-md custom-scrollbar text-left">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-[0.15em] text-white/50">
                  ...
                  <span className="text-lime-accent font-mono text-xs font-black">₹{maxPrice.toLocaleString('en-IN')}</span>
                </div>
                <input 
                  type="range" 
                  min="0" 
              ...
              {isLoading ? (
                ...
                <div className="border border-dashed border-white/10 bg-white/[0.01] rounded-2xl p-20 text-center backdrop-blur-md">
                  ...
                  <p className="text-xs text-white/40 max-w-xs mx-auto mt-1 leading-relaxed">
                    No timepieces match within this horological parameters profile layout.
                  </p>
                </div>
              ) : (
                        ...
                        <div className="pt-4 flex flex-col justify-between flex-grow space-y-4">
                          <div className="space-y-1">
                            ...
                            <p className="text-[11px] text-white/40 line-clamp-2 border-t border-white/5 pt-1.5 mt-1 leading-relaxed">
                              {product.description || 'Premium architectural engineering horology design assembly.'}
                            </p>
                          </div>
                          ...
                          <div className="flex items-center justify-between pt-1 border-t border-white/5">
                            <div className="flex flex-col">
                              ...
                              <div className="flex items-baseline gap-2">
                                <span className="text-lg font-black font-mono text-white">₹{offer.toLocaleString('en-IN')}</span>
                                {discount > 0 && (
                                  <span className="text-xs line-through text-white/30 font-bold">₹{original.toLocaleString('en-IN')}</span>
                                )}
                              </div>
                            </div>
