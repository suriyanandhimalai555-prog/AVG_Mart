...
const Navbar = () => {
    ...
    // Prevent background scrolling when full-screen mobile takeover is active
    useEffect(() => {
        if (isOpen) {
    ...
    // --- FETCH LIVE TOTAL QUANTITY FROM CART LEDGER ---
    useEffect(() => {
        if (!isLoggedIn) {
            setCartCount(0);
            return;
    ...
    return (
        <>
            {/* --- STANDARD REGULAR NAVBAR TIER --- */}
            <nav
                className={`fixed top-0 left-0 w-full z-40 px-6 py-4 md:px-12 transition-all duration-500 ${
                    isScrolled
                        ? 'bg-royal-main/95 backdrop-blur-xl border-b border-white/5 shadow-2xl py-3 text-white'
                        : 'bg-transparent border-b border-white/10 py-5 text-white'
                }`}
            >
                <div className="max-w-7xl mx-auto flex items-center justify-between relative">

                    {/* MOBILE TOGGLE BUTTON (Hidden entirely when takeover is open) */}
                    <div className={`flex md:hidden z-50 transition-opacity duration-300 ${isOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                        <button
                            onClick={() => setIsOpen(true)}
                            className="focus:outline-none p-1 text-white hover:text-lime-accent cursor-pointer"
                        >
                                                                ...
                                                                {cat.items.map((item) => (
                                                                    <li key={item}>
                                                                        <button
                                                                            ...
                                                                            className="text-lg text-white/60 hover:text-white font-medium flex items-center gap-1 group/item transition-colors w-full text-left cursor-pointer"
                                                                        >
                    ...
                    {/* BRAND LOGO DESIGN LAYOUT */}
                    <div onClick={() => { if (!isOpen) navigate("/"); }} className={`flex items-center gap-2.5 cursor-pointer select-none absolute left-1/2 -translate-x-1/2 md:static md:translate-x-0 z-50 transition-opacity duration-300 ${isOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                    ...
                    {/* SYSTEM UTILITIES TIER */}
                    <div className={`flex items-center space-x-2 md:space-x-4 z-50 transition-opacity duration-300 ${isOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                        ...
                        <div className="hidden md:block">
                            {isLoggedIn ? (
                                <button onClick={handleLogout} className="group inline-flex items-center gap-3 bg-lime-accent text-royal-dark px-6 py-3 font-black tracking-widest uppercase rounded-xl text-xs hover:bg-white transition-all duration-300 cursor-pointer">
                                    Logout <LogOut className="w-4 h-4" />
                                </button>
                            ) : (
                                <button onClick={() => navigate("/login")} className="group inline-flex items-center gap-3 bg-lime-accent text-royal-dark px-6 py-3 font-black tracking-widest uppercase rounded-xl text-xs hover:bg-white transition-all duration-300 cursor-pointer">
            ...
            {/* --- MATCHING TAKEOVER SYSTEM PORTAL LAYOUT --- */}
            <div
                className={`fixed inset-0 bg-royal-main text-white z-50 md:hidden flex flex-col justify-between transition-all duration-500 ease-in-out ${
                    isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-full pointer-events-none'
                }`}
            >
                {/* MATCHED FIXED SUB-HEADER ROW */}
                <div className="w-full flex items-center justify-between px-6 py-5 border-b border-white/5 bg-royal-dark/20">
                    ...
                    <button 
                        ...
                        className="p-2 rounded-full hover:bg-white/5 relative text-white/70 hover:text-white"
                    >
                        <ShoppingBag className="w-5 h-5" />
                        {cartCount > 0 && (
                            <span className="absolute top-1 right-1 w-2 h-2 rounded-full bg-lime-accent shadow-[0_0_8px_rgba(165,206,0,0.8)]" />
                        )}
                    </button>
                </div>

                {/* ACCORDION ACTION WRAPPER SECTIONS */}
                    ...
                    {navLinks.map((link, index) => {
                        ...
                        return (
                            <div key={link.name} className="border-b border-white/5 pb-3 pt-2">
                                ...
                                {hasDropdown && (
                                    <div
                                        className={`space-y-4 pt-3 pl-2 overflow-hidden transition-all duration-300 ${
                                            activeDropdown === index ? 'max-h-[350px] opacity-100' : 'max-h-0 opacity-0 pointer-events-none'
                                        }`}
                                    >
                                        <div className="py-0.5">
                                            <button 
                                                onClick={() => { setIsOpen(false); navigate('/allproducts'); }}
                                                className="text-[11px] font-black tracking-wider text-lime-accent uppercase inline-flex items-center gap-1 cursor-pointer"
                                            >
                                                ...
                                                <div className="grid grid-cols-2 gap-2">
                                                    {cat.items.map((item) => (
                                                        <button 
                                                            key={item} 
                                                            onClick={() => handleItemNavigation(item)} 
                                                            className="text-xs text-white/60 hover:text-white py-1 font-medium text-left w-full cursor-pointer"
                                                        >
                                                            {item}
                                                        </button>
                                                    ))}
                ...
                {/* BOTTOM FIXTURED CONTROLS MATRIX CONTAINER */}
                <div className="p-8 bg-royal-dark/40 border-t border-white/5 space-y-4">
                    <div className="flex items-center bg-white/5 border border-white/10 rounded-xl px-4 py-3.5 focus-within:border-white/30 transition-colors">
                        <Search className="w-4 h-4 text-white/40" />
                        <input
                            type="text"
                            placeholder="Search products..."
                            className="bg-transparent text-xs pl-3 outline-none w-full text-white placeholder-white/30 font-semibold"
                        />
                    </div>
                    ...
                    {isLoggedIn ? (
                        <button 
                            ...
                            className="w-full flex items-center justify-center gap-2 bg-red-500/10 border border-red-500/20 text-red-400 py-3.5 font-black tracking-widest uppercase rounded-xl text-xs hover:bg-red-500 hover:text-white transition-all duration-200 cursor-pointer"
                        >
                            Logout <LogOut className="w-4 h-4" />
                        </button>
                    ) : (
                        <button 
                            ...
                            className="w-full flex items-center justify-center gap-2 bg-lime-accent text-royal-dark py-3.5 font-black tracking-widest uppercase rounded-xl text-xs hover:bg-white transition-all duration-300 shadow-[0_4px_25px_rgba(165,206,0,0.3)] cursor-pointer"
                        >
                            Login <LogIn className="w-4 h-4" />
                        </button>
                    )}
